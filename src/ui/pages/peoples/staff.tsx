import React from "react";
import { FormatEmail } from "../../../utils/format.email";
import { useSelector } from "react-redux";
import { BACKEND_URL } from "../../../utils/api.constants";

const StaffMembers: React.FC = () => {
  const staff = useSelector((state: any) => state.staff);
  return (
    <>
      <div className="my-5 container">
        {" "}
        <div className="h1 text-center main_head ">Staff Members</div>
        <section className="container overflow-auto my-5">
          <table className="table table-bordered">
            <thead className="table-dark">
              <tr>
                <th>Sr. No.</th>
                <th>Profile Image</th>
                <th>Name</th>
                <th>Designation</th>
                <th>Email</th>
                <th>Mobile</th>
              </tr>
            </thead>
            <tbody>
              {!staff?.isLoading &&
                staff?.isSuccessful &&
                Object.keys(staff?.data).map((key: any, i: any) => {
                  return (
                    <tr key={i}>
                      <td>{i + 1}</td>
                      <td>
                        <a
                          href={
                            BACKEND_URL +
                            "/users/" +
                            staff?.data[key]?.profile_image
                              ? BACKEND_URL +
                                "/users/" +
                                staff?.data[key]?.profile_image
                              : "#"
                          }
                          target="_blank"
                        >
                          <img
                            src={
                              BACKEND_URL +
                              "/users/" +
                              staff?.data[key]?.profile_image
                                ? BACKEND_URL +
                                  "/users/" +
                                  staff?.data[key]?.profile_image
                                : "data:image/png;base64,"
                            }
                            alt="profile_image"
                            style={{
                              height: "150px",
                            }}
                            className="rounded-2 hover-scale-down"
                          />
                        </a>
                      </td>
                      <td>{staff?.data[key]?.name}</td>
                      <td>{staff?.data[key]?.designation}</td>
                      <td>
                        <a
                          href={"mailto:" + staff?.data[key]?.email}
                          target="_blank"
                        >
                          {FormatEmail(staff?.data[key]?.email)}
                        </a>
                      </td>
                      <td>{staff?.data[key]?.mobile}</td>
                    </tr>
                  );
                })}
            </tbody>
          </table>
        </section>
      </div>
    </>
  );
};

export default StaffMembers;
